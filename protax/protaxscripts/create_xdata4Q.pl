...
sub createX {
    my ($node, $n2cref, $n2rseqref, $n2rseqweightref, $seq_dont_use_ref, $qseqid, $remove_i, $unknoderef, $seqsimref, $n2prior, $unkprior, $debug_mode) = @_;
    ...
    my @X = (0, 0, 0, 0);
    # unk prior parameter x = p/(1-p) * sumprob  (this is for generality, sumprob needs not be 1)
   
    #my @xweight = (sprintf("%.6g",$unkprior/(1-$unkprior) * $sumprob));
    my @xweight = (1);

    my ($max,$eml,$nok,$nmissing, $wref);
    my $use_weighting;
    for ($i=0; $i<(scalar(@{$n2cref->{$node}})); $i++) {
	$cnode = $n2cref->{$node}->[$i];
	...
	if (exists($cnode_count{$i})) {
	    ...
	    ($max, $eml) = simMaxEml($qseqid, $n2rseqref->{$cnode}, $wref, $seqsimref, $seq_dont_use_ref);
	    push(@X, (0, 1, sprintf("%.6g",$max), sprintf("%.6g",$eml)));
	}
	else {
...
if (scalar(@ARGV) < 7) {
    die "usage: create_xdataMaxEml.pl unkprior trainsample_file taxonomyfile seqid2taxonomy_file taxonomy2rseqid_file seqsimfile outfile (debug_mode 0,1,2)\n";
}
...
while (<FD>) {
    ...
    # create X matrix for parent node

    my $pnode = $n2pref->{$node};
    my @xdat=($weight, $priprob);
    my ($x, $xwref) = createX($pnode, $n2cref, $n2ridref, $n2rweightref, \%seq_dont_use, $trainseq, $remove_i, $unknoderef, $seqsimref, $n2prior, $unkprior, $debug_mode);
