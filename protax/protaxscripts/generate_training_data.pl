...
for (my $i=1; $i<=$num_trainseqs; $i++) {
    ...
    $priprob = 0.95 * $priprob / $n2prior->{$n2pref->{$node}};
	...
	# find nearest node with >=1 seqs: pick one seq, none to reference --> all don't use
        # instead of 1, find nearest node with >=2 seqs (this quarantees along path from parent node to root it is possible to reach current node)
	$node = findNearestSameLevelNode($node,$n2pref,$n2cref,$n2lref,$n2sidref,$n2ndref,2,$n2prior);
	@seqs = @{$n2sidref->{$node}};
	$ri = int(rand(scalar(@seqs)));
	$trainseq = $seqs[$ri];
